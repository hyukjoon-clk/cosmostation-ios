...
extension CosmosFetcher {
    ...
    func fetchBalance() async throws -> [Cosmos_Base_V1beta1_Coin]? {
        if (getEndpointType() == .UseGRPC) {
            let page = Cosmos_Base_Query_V1beta1_PageRequest.with {
                $0.limit = chain.apiName == "sei" ? 1000 : 2000
            }
            ...
            let limit = chain.apiName == "sei" ? 1000 : 2000
            let url = getLcd() + "cosmos/bank/v1beta1/balances/${address}?pagination.limit=\(limit)"
                .replacingOccurrences(of: "${address}", with: chain.bechAddress!)
    ...
    func fetchSpendableBalance() async throws -> [Cosmos_Base_V1beta1_Coin]? {
        if (getEndpointType() == .UseGRPC) {
            ...
            let limit = chain.apiName == "sei" ? 1000 : 2000
            let url = getLcd() + "cosmos/bank/v1beta1/spendable_balances/${address}?pagination.limit=\(limit)"
                .replacingOccurrences(of: "${address}", with: chain.bechAddress!)
